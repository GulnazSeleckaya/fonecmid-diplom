...
Процедура СформироватьДвижениеОсновныеНачисления()
	...
	Для Каждого Строка Из ОсновныеНачисления Цикл
		...
		Если Строка.ВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Отпуск Тогда
			...
			Движение.Сотрудник = Строка.Сотрудник;
			Движение.КоличествоДней = (КонецДня(Строка.ДатаОкончания) - Строка.ДатаНачала) / 86400;
...
Процедура РассчитатьПроцентОтРабот()
	...
	Запрос.Текст = 
		...
		|УПОРЯДОЧИТЬ ПО
		|	ВТ_ДанныеДокумента.НомерСтроки ВОЗР";
	...
	Пока Выборка.Следующий() Цикл 
		...
		Если Индекс >= 0 И Индекс < Движения.ВКМ_ОсновныеНачисления.Количество() Тогда
			Движение = Движения.ВКМ_ОсновныеНачисления[Индекс];
...
Процедура ВзаиморасчетыССотрудниками()
	...
	Запрос.Текст =
		...
		|	ОсновныеНачисления.Результат КАК СуммаНачислений,
		...
		|	Удержания.Сумма КАК СуммаУдержаний
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ВсеСуммы.Сотрудник КАК Сотрудник,
		|	СУММА(ВТ_ВсеСуммы.СуммаНачислений) КАК СуммаНачислений,
		|	СУММА(ВТ_ВсеСуммы.СуммаУдержаний) КАК СуммаУдержаний
		|ИЗ
		|	ВТ_ВсеСуммы КАК ВТ_ВсеСуммы
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ВсеСуммы.Сотрудник";
